...
// Function to fetch dilemma content from Firestore
export async function fetchDilemmaContent(dilemmaId) {
	try {
		const docRef = doc(db, "dilemmas", dilemmaId);
		...
		if (docSnap.exists()) {
			...
			console.log("No dilemma found for date:", dilemmaId);
			// Return default dilemma if none exists for this date
			return {
				optionA: "Option A 🤔",
				optionB: "Option B 🤯",
			};
		...
		console.error("Error fetching dilemma content:", error);
		// Return fallback dilemma on error
		return {
			optionA: "Option A 🤔",
			optionB: "Option B 🤯",
		};
